...
test('deepEqual', (assert) => {
  ...
  assert.throws(() => _assert.deepEqual([1], []), 'should throw when different lengths');

  assert.deepEqual([1, 2], [1, 2], 'two arrays with the same value');

  assert.throws(() => _assert.deepEqual([2, 1], [1, 2]), 'two arrays with different order should throw');
  ...
  assert.throws(() => _assert.deepEqual({ a: 1, c: 2 }, { a: 1, b: 2 }),
    'two objects with different attributes should throw');
